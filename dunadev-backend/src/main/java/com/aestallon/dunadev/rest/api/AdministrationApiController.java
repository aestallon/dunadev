...
public class AdministrationApiController implements AdministrationApi {
    ...
    private final AdministrationApiDelegate delegate;

    public AdministrationApiController(@Autowired(required = false) AdministrationApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new AdministrationApiDelegate() {});
    }

    @Override
    public AdministrationApiDelegate getDelegate() {
        return delegate;
    }
